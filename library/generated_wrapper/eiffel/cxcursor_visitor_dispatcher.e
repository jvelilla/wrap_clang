...
class CXCURSOR_VISITOR_DISPATCHER

inherit

	EWG_CLANG_CALLBACK_C_GLUE_CODE_FUNCTIONS_API
		export {NONE} all end
	DISPOSABLE
...
feature -- Initialization
	...
	make
		...
		do
			set_cxcursor_visitor_object ($Current)
		end

feature -- Access: Routine 

	routine_1: detachable FUNCTION [TUPLE [a_cursor: POINTER; a_parent: POINTER; a_client_data: POINTER], INTEGER] 
			--Eiffel routine to be call on callback.

	routine_2: detachable FUNCTION [TUPLE [a_cursor: POINTER; a_parent: POINTER; a_client_data: POINTER], INTEGER] 
			--Eiffel routine to be call on callback.

	routine_3: detachable FUNCTION [TUPLE [a_cursor: POINTER; a_parent: POINTER; a_client_data: POINTER], INTEGER] 
			--Eiffel routine to be call on callback.
...
feature -- Access: Dispatcher
	...
	c_dispatcher_1: POINTER
		...
		do
			Result := get_cxcursor_visitor_stub_1
		end
	...
	c_dispatcher_2: POINTER
		...
		do
			Result := get_cxcursor_visitor_stub_2
		end
	...
	c_dispatcher_3: POINTER
		...
		do
			Result := get_cxcursor_visitor_stub_3
		end

feature -- Access: Callback

	on_callback_1 (a_cursor: POINTER; a_parent: POINTER; a_client_data: POINTER): INTEGER  
			-- Callback target.
		do
			if attached routine_1 as l_routine then 
				Result :=  l_routine (a_cursor, a_parent, a_client_data)
			end
		end

	on_callback_2 (a_cursor: POINTER; a_parent: POINTER; a_client_data: POINTER): INTEGER  
			-- Callback target.
		do
	...
	on_callback_3 (a_cursor: POINTER; a_parent: POINTER; a_client_data: POINTER): INTEGER  
			-- Callback target.
		do
...
feature -- Register: Callbacks
	...
	register_callback_1 (a_routine: like routine_1)
		...
		do
			routine_1 := a_routine
			set_cxcursor_visitor_entry_1 ($on_callback_1)
		ensure
	...
	register_callback_2 (a_routine: like routine_1)
		...
		do
			routine_2 := a_routine
			set_cxcursor_visitor_entry_2 ($on_callback_2)
		ensure
	...
	register_callback_3 (a_routine: like routine_1)
		...
		do
			routine_3 := a_routine
			set_cxcursor_visitor_entry_3 ($on_callback_3)
		ensure
...
feature {NONE} -- Implementation
	...
	dispose
		...
		do
			...
			set_cxcursor_visitor_object (default_pointer)
		end
